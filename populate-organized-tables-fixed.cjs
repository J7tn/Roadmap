...
async function populateIndustries() {
  ...
  // Insert industry translations (without description column for now)
...
async function populateCareers() {
  ...
  // Get existing industries to validate foreign keys
  const { data: existingIndustries } = await supabase
    .from('industries_new')
    .select('id');

  const validIndustryIds = new Set(existingIndustries?.map(i => i.id) || []);

  const careersToInsert = careers
  ...
  console.log(`   📊 Inserting ${careersToInsert.length} careers (filtered from ${careers.length} total)...`);
  
  if (careersToInsert.length === 0) {
    console.log('   ⚠️ No careers to insert - check industry mappings');
    return;
  }
...
async function populateCareerContent() {
  ...
  // Get existing careers to validate foreign keys
  const { data: existingCareers } = await supabase
    .from('careers_new')
    .select('id');

  const validCareerIds = new Set(existingCareers?.map(c => c.id) || []);

  const contentToInsert = careerTranslations
    .filter(translation => validCareerIds.has(translation.career_id))
    .map(translation => ({
  ...
  console.log(`   📊 Inserting ${contentToInsert.length} career content entries (filtered from ${careerTranslations.length} total)...`);

  if (contentToInsert.length === 0) {
    console.log('   ⚠️ No career content to insert - check career IDs');
    return;
  }
...
async function populateCareerTrends() {
  ...
  console.log(`   📊 Inserting ${trendsToInsert.length} career trends (filtered from ${trends.length} total)...`);

  if (trendsToInsert.length === 0) {
    console.log('   ⚠️ No career trends to insert - check career IDs');
    return;
  }

  // Insert trends one by one to avoid conflicts
  let successCount = 0;
  let errorCount = 0;
  ...
  for (const trend of trendsToInsert) {
    try {
      const { error } = await supabase
        ...
        .upsert([trend], { onConflict: 'career_id' });
      
      if (error) {
        console.error(`   ❌ Error inserting trend for career ${trend.career_id}:`, error.message);
        errorCount++;
      } else {
        successCount++;
      }
    } catch (error) {
      console.error(`   ❌ Exception inserting trend for career ${trend.career_id}:`, error.message);
      errorCount++;
    }
  }

  console.log(`   ✅ Career trends populated: ${successCount} successful, ${errorCount} errors`);
}
